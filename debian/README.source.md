...
it, simply run:
    ...
    gbp clone --add-upstreamvcs vcs-git:galera-4
...
remote, and fetches the latest upstream `4.x` branch commits and tags. The
...
above. Then open https://salsa.debian.org/mariadb-team/galera-4 and press
...
`git@salsa.debian.org:otto/galera-4.git`, and add it as new remote (replace
'otto' with your own Salsa username):

    git remote add otto git@salsa.debian.org:otto/galera-4.git
...
To check for new upstream releases run:
    ...
    # Note latest release tag, e.g. release_26.4.21, and verify it
    git verify-tag release_26.4.21
...
You may also want to open a new changelog entry for the next revision:

    debchange --increment --vendor=debian \
      "Open new changelog entry. This will be replaced by 'gbp dch'."
    git commit -m "Open new changelog entry" -m "Gbp-Dch: Ignore" debian/changelog
